function drawControlPointStatus(argument0) {
	// Global "parameters":
	...
	for (i=1; i<= global.totalControlPoints; i+=1)
	{
	    ...
	    if (global.cp[i].locked)
	    {
	        if (cpUnlock >= 150 or cpUnlock == 0)
	            ...
	            draw_text_transformed(drawx+2, yoffset+ysize-38, string_hash_to_newline(ceil(cpUnlock/30)), 3, 3, 0);
	    }
	    else if (global.cp[i].cappers > 0 and not global.cp[i].locked)
	    {
	        ...
	        draw_text_transformed(drawx+13, yoffset+ysize-38, string_hash_to_newline(string(global.cp[i].cappers)), 1.5, 1.5, 0);
	    }
	    drawx += 60;
	}
